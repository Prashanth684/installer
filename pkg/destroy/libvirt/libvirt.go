		...
		if err := domain.UndefineFlags(libvirt.DOMAIN_UNDEFINE_NVRAM); err != nil {
			if e := err.(libvirt.Error); e.Code == libvirt.ERR_NO_SUPPORT || e.Code == libvirt.ERR_INVALID_ARG {
				...
				if err := domain.Undefine(); err != nil {
					return false, errors.Wrapf(err, "couldn't undefine libvirt domain: %q", dName)
				}
			} else {
				return false, errors.Wrapf(err, "couldn't undefine libvirt domain %q with flags", dName)
			}
		}
